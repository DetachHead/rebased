...
open class UpdateStrategyCustomization {
  ...
  open fun forceEapUpdateChannelForEapBuilds(): Boolean = true

  /**
   * Executed when the IDE starts and provides a way to change the current update channel.
   * If a non-null value is returned, it'll be used as the current update channel as is.
   * If `null` is returned, the default customization will be applied.
   *
   * By default, the update channel is set to ['EAP'][ChannelStatus.EAP] for EAP builds if [.forceEapUpdateChannelForEapBuilds] is `true`,
   * and to ['release'][ChannelStatus.RELEASE] for release builds on the first run.
   */
  open fun changeDefaultChannel(currentChannel: ChannelStatus): ChannelStatus? = null

  open fun isChannelActive(channel: ChannelStatus): Boolean = channel != ChannelStatus.MILESTONE && channel != ChannelStatus.BETA
   ...
   * new builds are available to suggest updating to the build from the same major version, i.e., IntelliJ IDEA 2018.2.5 will suggest
   * updating to 2018.2.6, not to 2018.3.
   *
   * Override this method if major versions of your IDE don't directly correspond to the major version of the IntelliJ platform.
   */
  open fun haveSameMajorVersion(build1: BuildNumber, build2: BuildNumber): Boolean = build1.baselineVersion == build2.baselineVersion

  /**
   * Returns `true` if the version of `candidateBuild` is newer than the version of `currentBuild` – so it may be suggested as an update.
   *
   ...
   */
  open fun isNewerVersion(candidateBuild: BuildNumber, currentBuild: BuildNumber): Boolean = candidateBuild > currentBuild
  ...
  open fun isChannelApplicableForUpdates(updateChannel: UpdateChannel, selectedChannel: ChannelStatus): Boolean =
    updateChannel.status >= selectedChannel
   ...
   * to the new version.
   */
  open fun canBeUsedForIntermediatePatches(updateChannel: UpdateChannel, selectedChannel: ChannelStatus): Boolean = true

  /**
   * Returns `null` if user should be allowed to change the update channel in UI. If a non-null value is returned,
